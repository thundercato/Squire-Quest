...
sqCmd('SqDungeonTalkGuard', /^(?:talk|speak|chat)(?: to| with)? (?:the )?(?:guard|jailer|gaoler|prison guard)$/i, function () {
  if (!sqHere('dungeon_cell')) return sqFail('The prison guard is not here.')
  if (!sq.dungeonAwake) { sq.dungeonAwake = true; w.sq_princess.loc = 'dungeon_cell'; return sqMsg('The guard gestures around the cell. “Four walls, straw bed, locked door. Welcome to your worst Knightmare.” Before you can object to the pun, the Princess arrives with an escort. She recognises you from the forest and thanks you again. She is also openly relieved Sir Arrogant did not win.') }
...
sqCmd('SqGiveBroochPrincess', [/^(?:give|hand|offer) (?:the )?(?:brooch|jewel(?:led|-encrusted)? brooch) to (?:the )?princess$/i,/^(?:give|hand) (?:the )?princess (?:the )?(?:brooch|jewel(?:led|-encrusted)? brooch)$/i], function () {
  if (!sqHere('dungeon_cell') || !sq.dungeonAwake || w.sq_princess.loc !== 'dungeon_cell') return sqFail('The Princess is not currently here to receive it.')
...
sqCmd('SqSleepDungeon', /^(?:sleep|sleep on|lie on|lie down on|rest on|go to sleep(?: on)?) ?(?:the )?(?:straw|hay|bed)?$/i, function () {
  if (!sqHere('dungeon_cell')) return sqFail('This is not a good sleeping location, which is saying something.')
...
sqCmd('SqPushDungeonWall', /^(?:push|shove|press|move|ram|kick) (?:the )?(?:wall|stonework|stones|stone|loose stone|loose stonework|west wall)$/i, function () {
  if (!sqHere('dungeon_cell')) return sqFail('There is no useful loose prison wall here.')
...
sqCmd('SqTakeFemur', /^(?:take|get|grab|pull|pick up) (?:the )?(?:femur|bone|leg bone)$/i, function () {
  if (!sqHere('dungeon_cell_west')) return sqFail('There is no useful femur here.')
...
sqCmd('SqTakeRags', /^(?:take|get|grab|pick up|collect) (?:the )?(?:rags|cloth|clothes|old clothes|old rags)$/i, function () {
  if (!sqHere('dungeon_cell_west')) return sqFail('There are no old prison rags here.')
...
sqCmd('SqWetRags', [/^(?:wet|soak|dampen|dip) (?:the )?(?:rags|cloth|clothes) (?:in|under|with) (?:the )?(?:water|drip|dripping water)$/i,/^(?:put|hold) (?:the )?(?:rags|cloth|clothes) (?:under|in) (?:the )?(?:water|drip|dripping water)$/i], function () {
  if (!sqHere('dungeon_cell')) return sqFail('You need the dripping water in the starting cell.')
...
sqCmd('SqWrapVent', /^(?:wrap|tie|loop|use|put|place) (?:the )?(?:wet )?(?:rags|cloth|clothes) (?:around|round|on|over|through) (?:the )?(?:bars|vent|grille|grill)$/i, function () {
  if (!sqHere('dungeon_cell')) return sqFail('There is no barred vent here.')
...
sqCmd('SqUseFemurVent', /^(?:use|put|insert|place|stick) (?:the )?(?:femur|bone|leg bone) (?:in|into|through|with|on) (?:the )?(?:rags|bars|vent|cloth)$/i, function () {
  if (!sqHere('dungeon_cell')) return sqFail('The vent is not here.')
...
sqCmd('SqTwistFemur', [/^(?:twist|turn|rotate|wind|crank) (?:the )?(?:femur|bone|leg bone)$/i,/^(?:twist|turn|rotate|wind|crank) (?:the )?(?:rags|vent|bars)$/i], function () {
  if (!sqHere('dungeon_cell') || !sq.ventWrapped || !sqHas('sq_femur')) return sqFail('You have not assembled the wet-rag-and-femur leverage device yet.')
...
sqCmd('SqTakeTorch', /^(?:take|get|grab|pick up) (?:the )?(?:torch|flaming torch)$/i, function () {
  if (!sqHere('dungeon_cell')) return sqFail('There is no torch here.')
...
sqCmd('SqEnterVent', [/^(?:enter|climb into|crawl into|go into|squeeze into|use) (?:the )?(?:vent|shaft|vent shaft)$/i,/^(?:go|climb|crawl) up$/i], function () {
  if (!sqHere('dungeon_cell')) return sqFail('There is no accessible dungeon vent here.')
  ...
  return sqMovePlayer('vent_shaft_start','You squeeze into the vent with the torch between your teeth. Dignity remains behind in the cell.')
})
sqCmd('SqVentWest', /^(?:go|crawl|move|head) west$/i, function () {
  if (!['vent_shaft_start','vent_shaft'].includes(player.loc)) return sqFail('West is not a special vent route here.')
  ...
  return sqHere('vent_shaft_start') ? sqMovePlayer('vent_shaft') : sqMovePlayer('large_cavern')
})
...
settings.afterEnter = function () {
  ...
  if (player.loc==='dungeon_cell' && sq.stage==='dungeon' && !sq.dungeonAwake) {
    sq.dungeonAwake=true; w.sq_princess.loc='dungeon_cell'
